  ...
  <header class="nav select-none" text-m flex-row-bt-c px-2 md:px-6 dark:text="light">
    ...
    <div class="group left flex-row-c-c">
      <NuxtLink mx-2 to="/" flex-row-c-c class="relative">
        <img
          ...
          transition-300 dark:hidden group-hover:opacity-85 group-hover:filter-blur-2px
        >
        <img
          loading="lazy" alt="Design By Kiwi23333" src="@/assets/images/logo/logo_dark.png" hidden w-2.4rem
          object-contain transition-300 dark:block group-hover:opacity-85 group-hover:filter-blur-2px
        >
        <span
      ...
      <a class="group" target="_blank" href="https://github.com/KiWi233333">
        <img
          loading="lazy" ml-4 w-5rem opacity-0 transition-300 group-hover:opacity-100 dark:filter-invert-100
    ...
    <ClientOnly>
      <div class="right flex-row-c-c">
        ...
        <!-- 购物车 -->
        <NuxtLink to="/user/shopcart" class="hidden md:block hover:animate-[shopcart_1s_ease-in]">
          <i
        ...
        <BtnSwitch />
        <!-- 登陆注册 -->
        <div v-if="isLogin" class="group z-0 flex flex-col">
          <!-- 移动端 -->
          <div class="block md:hidden">
            <el-popover
              placement="bottom" class="inline-block sm:hidden" teleported :width="100" :offset="20"
              trigger="click"
            >
              <template #reference>
                <el-button class="sm:hidden" round style="margin: 0; padding: 0.5em">
                  <i
